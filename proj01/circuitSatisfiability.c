 ...
 *   Downloaded and edited by Bryce Allen, for cs374 project 1, Calvin University.
 */

#include <stdio.h>     // printf()
#include <limits.h>    // UINT_MAX
#include <mpi.h>       //added code here by Bryce Allen
...
int main (int argc, char *argv[]) {
   ...
   //int localCount = 0;
   double startTime = 0.0, totalTime = 0.0;//added code here by Bryce Allen
   

   MPI_Init(&argc, &argv); //argv is a list of arguments, in which is the number 
                           //of processes desired by the user. Which the function
                           //then spawns off that number.

   startTime = MPI_Wtime();//added code here by Bryce Allen

	MPI_Comm_rank(MPI_COMM_WORLD, &id); //assigns each respective process executing this line an id.
   ...
   totalTime = MPI_Wtime() - startTime;//added code here by Bryce Allen
   MPI_Finalize();

   

   printf ("Process %d finished in time %f secs.\n", id, totalTime); //modified line by Bryce Allen
   fflush (stdout);
   if(id == 0)
